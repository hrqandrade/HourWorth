...
struct HomeView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: HWSpacing.extraLarge) {
                ...
                sectionHeader(Localizable.Home.activeProjects, detail: Localizable.Home.projectCount(active.count))
                if active.isEmpty { EmptyState(icon: "briefcase", title: Localizable.Home.noActiveProjects, message: Localizable.Home.noActiveProjectsMessage) }
                ...
                sectionHeader(Localizable.Home.needsAttention, detail: Localizable.Home.nextDeadline)
    ...
    private var hero: some View {
        ...
        return VStack(alignment: .leading, spacing: HWSpacing.card) {
            Text(Localizable.Home.thisMonth).font(.caption.bold()).foregroundStyle(.white.opacity(0.7))
            Text(received.money).font(.system(size: 40, weight: .bold)).monospacedDigit().foregroundStyle(.white)
            Text(Localizable.Home.receivedContext).foregroundStyle(.white.opacity(0.75))
